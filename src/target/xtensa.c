...
/* Special register number macro for DDR, PS, A3, A4 registers.
...
const struct xtensa_keyval_info_s xt_qerr[XT_QERR_NUM] = {
	{ "E00", ERROR_FAIL },
	{ "E01", ERROR_FAIL },
	{ "E02", ERROR_COMMAND_ARGUMENT_INVALID },
	{ "E03", ERROR_FAIL },
};
		...
		if (xtensa->core_config->windowed) {
			...
			for (enum xtensa_ar_scratch_set_e s = 0; s < XT_AR_SCRATCH_NUM; s++) {
				if (strcmp(reg->name, xtensa->scratch_ars[s].chrval) == 0) {
					LOG_DEBUG("Scratch reg %s [0x%08x] set from gdb", reg->name, value);
...
static enum xtensa_reg_id xtensa_windowbase_offset_to_canonical(struct xtensa *xtensa, 
																const enum xtensa_reg_id reg,
																const int windowbase)
{
	int idx;
	if (reg >= XT_REG_IDX_AR0 && reg <= XT_REG_IDX_ARLAST)
...
static int xtensa_write_dirty_registers(struct target *target)
{
	...
	if (xtensa->core_config->windowed) {
		...
		for (i = XT_REG_IDX_A0; i <= XT_REG_IDX_A15; i++) {
			j = xtensa_windowbase_offset_to_canonical(xtensa, i, windowbase);
	...
	if (xtensa->core_config->windowed) {
		...
		for (j = 0; j < XT_REG_IDX_ARLAST; j += 16) {
...
int xtensa_examine(struct target *target)
{
	...
	cmd = PWRCTL_DEBUGWAKEUP(xtensa)|PWRCTL_MEMWAKEUP(xtensa)|PWRCTL_COREWAKEUP(xtensa);
...
{
	struct xtensa *xtensa = target_to_xtensa(target);
	int cmd = PWRCTL_DEBUGWAKEUP(xtensa)|PWRCTL_MEMWAKEUP(xtensa)|PWRCTL_COREWAKEUP(xtensa);
...
int xtensa_deassert_reset(struct target *target)
{
	...
	xtensa_queue_pwr_reg_write(xtensa,
		XDMREG_PWRCTL,
		PWRCTL_JTAGDEBUGUSE(xtensa)|PWRCTL_DEBUGWAKEUP(xtensa)|PWRCTL_MEMWAKEUP(xtensa)|PWRCTL_COREWAKEUP(xtensa));
	xtensa_dm_queue_tdi_idle(&xtensa->dbg_mod);
	res = xtensa_dm_queue_execute(&xtensa->dbg_mod);
	if (res != ERROR_OK)
		return res;
...
int xtensa_fetch_all_regs(struct target *target)
{
	...
	for (j = 0; j < XT_AREGS_NUM_MAX; j += 16) {
		...
		for (i = 0; i < 16; i++) {
			if (i+j < xtensa->core_config->aregs_num) {
				...
				xtensa_queue_dbg_reg_read(xtensa, XDMREG_DDR, &regvals[XT_REG_IDX_AR0 + i + j]);
				xtensa_queue_dbg_reg_read(xtensa, XDMREG_DSR, &dsrs[XT_REG_IDX_AR0 + i + j]);
			}
		}
		if (xtensa->core_config->windowed) {
			/*Now rotate the window so we'll see the next 16 registers. The final rotate
	...
	if (xtensa->core_config->coproc) {
		 ...
		 * */
		xtensa_queue_exec_ins(xtensa,
			XT_INS_RSR(xtensa, xtensa_regs[XT_REG_IDX_CPENABLE].reg_num, XT_REG_A3));
		xtensa_queue_exec_ins(xtensa, XT_INS_WSR(xtensa, XT_SR_DDR, XT_REG_A3));
		xtensa_queue_dbg_reg_read(xtensa, XDMREG_DDR, &regvals[XT_REG_IDX_CPENABLE]);
	}
	LOG_DEBUG("%s: CPENABLE done", target_name(target));
	res = xtensa_dm_queue_execute(&xtensa->dbg_mod);
	if (res != ERROR_OK) {
		LOG_ERROR("Failed to read ARs (%d)!", res);
		return res;
	}
	LOG_DEBUG("%s: call 1 xtensa_core_status_check()", target_name(target));
	xtensa_core_status_check(target);
	LOG_DEBUG("%s: xtensa_core_status_check() complete", target_name(target));

	if (xtensa->core_config->coproc)
		cpenable = (int)buf_get_u32((uint8_t *)&regvals[XT_REG_IDX_CPENABLE], 0, 32);
	...
	LOG_DEBUG("%s: SFRs done", target_name(target));
	...
	LOG_DEBUG("%s: call 2 xtensa_core_status_check()", target_name(target));
	xtensa_core_status_check(target);
	LOG_DEBUG("%s: xtensa_core_status_check() complete", target_name(target));
	...
	LOG_DEBUG("%s: DSR check done", target_name(target));
	...
	for (i = 0; i < reg_list_size; i++) {
		...
		if (xtensa_reg_is_readable(rlist[ridx].flags, cpenable) && rlist[ridx].exist) {
			if ((xtensa->core_config->windowed) && (rlist[ridx].type == XT_REG_GENERAL)) {
				 ...
				 * We need to get the real register address by subtracting windowbase
				 * and wrapping around. */
	...
	enum target_register_class reg_class)
{
	...
	if (reg_class == REG_CLASS_GENERAL) {
        LOG_ERROR("reg_class %d unhandled", reg_class);
        return ERROR_FAIL;
    }

    *reg_list_size = xtensa->dbregs_num;
	...
	for (i = 0, k = 0; i < (int)xtensa->core_cache->num_regs && k < *reg_list_size; i++) {
		if (xtensa->core_cache->reg_list[i].exist) {
			...
			if (i == XT_REG_IDX_PS) {
				...
				//LOG_DEBUG("SPARSE GDB reg 0x%x getting EPS%d 0x%x", sparse_idx, xtensa->core_config->debug.irq_level, xtensa_reg_get_value((*reg_list)[sparse_idx]));
			} else if (rlist[ridx].type == XT_REG_RELGEN) {
...
int xtensa_halt(struct target *target)
{
	...
	if (!xtensa_is_stopped(target)) {
		LOG_DEBUG("DSR.Stopped not set (DSR 0x%08x). Set DCR.[EnableOCD|DebugInterrupt] (0x%08x)...",
				xtensa_dm_core_status_get(&xtensa->dbg_mod), OCDDCR_ENABLEOCD|OCDDCR_DEBUGINTERRUPT);
		xtensa_queue_dbg_reg_write(xtensa,
			XDMREG_DCRSET,
			OCDDCR_ENABLEOCD|OCDDCR_DEBUGINTERRUPT);
		xtensa_dm_queue_tdi_idle(&xtensa->dbg_mod);
		...
		LOG_DEBUG("DSR.Stopped set (DSR 0x%08x).", xtensa_dm_core_status_get(&xtensa->dbg_mod));
	}
	...
	int handle_breakpoints)
{
	...
	if ((oldps & 0xf) >= icountlvl) {
		...
		LOG_DEBUG("Lowering PS.INTLEVEL to allow stepping: %s <- 0x%08x (was 0x%08x)",
	...
	do {
		...
		if (xtensa->core_config->windowed &&
			...
			LOG_INFO("Stepping out of window exception, PC=%X", cur_pc);
			oldpc = cur_pc;
			address = oldpc + 3;
			continue;
		}
	...
	int access)
{
	...
	if (mem_region)
		return (mem_region->access & access) == access;
	mem_region = xtensa_memory_region_find(&xtensa->core_config->sram, address);
	if (mem_region)
		return (mem_region->access & access) == access;
	mem_region = xtensa_memory_region_find(&xtensa->core_config->srom, address);
	if (mem_region)
		return (mem_region->access & access) == access;
	return true;
}
...
/*  LOG_INFO("Converted to aligned addresses: read from %08X to %08X", addrstart_al, addrend_al); */
	...
	if (xtensa->probe_lsddr32p != 0) {
		...
		while (adr != addrend_al) {
			...
			xtensa_queue_dbg_reg_read(xtensa, XDMREG_DDR, (uint32_t *)&albuff[i]);
			adr += sizeof(uint32_t);
			i += sizeof(uint32_t);
	...
	if (bswap) {
		/* TODO: could combine swap and memcpy for some cases */
...
/*  LOG_INFO("al start %x al end %x", addrstart_al, addrend_al); */
	...
	if (fill_head_tail) {
		...
		if (address & 3) {
			...
			xtensa_queue_dbg_reg_read(xtensa, XDMREG_DDR, (uint32_t *)&albuff[0]);
		}
		if ((address + (size*count)) & 3) {
			...
			xtensa_queue_dbg_reg_read(xtensa, XDMREG_DDR, (uint32_t *)&albuff[addrend_al-addrstart_al-4]);
		}
		/*Grab bytes */
		res = xtensa_dm_queue_execute(&xtensa->dbg_mod);
		if (res != ERROR_OK) {
	...
	if (res != ERROR_OK) {
		...
		if (issue_ihi || issue_dhwb) {
			...
			xtensa_core_status_check(target);
			if (res != ERROR_OK) {
				LOG_ERROR("Error issuing cache writeback/invaldate instruction(s): %d", res);
			}
		}
	}
	if (albuff != buffer)
...
int xtensa_poll(struct target *target)
{
	...
	if (xtensa->dbg_mod.power_status.stat != xtensa->dbg_mod.power_status.stath) {
		LOG_DEBUG("PWRSTAT: read 0x%08x, clear 0x%08x, reread 0x%08x",
	...
	if (prev_dsr != xtensa->dbg_mod.core_status.dsr) {
		LOG_DEBUG("DSR has changed: 0x%08x", xtensa->dbg_mod.core_status.dsr);
	}
	if (xtensa->dbg_mod.power_status.stath & PWRSTAT_COREWASRESET(xtensa)) {
		...
		if (target->state != TARGET_HALTED) {
			...
			LOG_DEBUG("%s: Target halted, pc=0x%08X, debug_reason=%08x, oldstate=%08x",
				...
				target->debug_reason,
				oldstate);
			LOG_DEBUG("%s: [regs] debugcause=0x%08X, exccause=%d, dsr=0x%08x",
				target_name(target),
				halt_cause,
				xtensa_reg_get(target, XT_REG_IDX_EXCCAUSE),
...
static int xtensa_update_instruction(struct target *target, 
	target_addr_t address, 
	uint32_t size,
	const uint8_t *buffer)
{
	...
	if (issue_ihi || issue_dhwbi) {
		...
		xtensa_queue_exec_ins(xtensa, XT_INS_RSR(xtensa, XT_SR_DDR, XT_REG_A3));
		LOG_DEBUG("DHWBI, IHI for address "TARGET_ADDR_FMT, address);
		if (issue_dhwbi) {
			xtensa_queue_exec_ins(xtensa, XT_INS_DHWBI(xtensa, XT_REG_A3, 0));
			if (!same_dc_line) {
				LOG_DEBUG("DHWBI second dcache line for address "TARGET_ADDR_FMT, address + 4);
		...
		if (issue_ihi) {
			...
			if (!same_ic_line) {
				LOG_DEBUG("IHI second icache line for address "TARGET_ADDR_FMT, address + 4);
	...
	if (ret != ERROR_OK) {
		LOG_ERROR("Error writing instruction to memory: %d", ret);
		return ret;
	}
	...
	if (issue_dhwbi) {
		...
		if (!same_dc_line) {
			LOG_DEBUG("DHWB second dcache line for address "TARGET_ADDR_FMT, address + 4);
			xtensa_queue_exec_ins(xtensa, XT_INS_DHWB(xtensa, XT_REG_A3, 4));
		}

		/* Sync instruction pipeline */
		xtensa_queue_exec_ins(xtensa, XT_INS_ISYNC(xtensa));
	...
	struct xtensa_sw_breakpoint *sw_bp)
{
	...
	break_insn.d32 = sw_bp->insn_sz == XT_ISNS_SZ_MAX ? XT_INS_BREAK(xtensa, 0, 0) : XT_INS_BREAKN(xtensa, 0);

    ret = xtensa_update_instruction(target, breakpoint->address, sw_bp->insn_sz, break_insn.d8);
	if (ret != ERROR_OK) {
...
static int xtensa_sw_breakpoint_remove(struct target *target, struct xtensa_sw_breakpoint *sw_bp)
{
    ...
    ret = xtensa_update_instruction(target, sw_bp->oocd_bp->address, sw_bp->insn_sz, sw_bp->insn);
	if (ret != ERROR_OK) {
		LOG_ERROR("%s: Failed to write insn (%d)!", target_name(target), ret);
		return ret;
	}
	sw_bp->oocd_bp = NULL;
	return ERROR_OK;
...
int xtensa_breakpoint_add(struct target *target, struct breakpoint *breakpoint)
{
	...
	LOG_DEBUG("%s: placed HW breakpoint %u @ " TARGET_ADDR_FMT,
		target_name(target),
		slot,
		breakpoint->address);
...
int xtensa_build_reg_cache(struct target *target)
{
    ...
    int last_dbreg_num = 0;

	if (xtensa->core_regs_num + num_xtensa_optregs != xtensa->total_regs_num) {
		LOG_WARNING("Register count MISMATCH: %d core regs, %d extended regs; %d expected", 
	...
	reg_cache->next = NULL;
	/* Init reglist */
	int reg_list_size = XT_NUM_REGS + num_xtensa_optregs;
	...
	for (int whichlist = 0; whichlist < 2; whichlist++) {
		...
		for (uint32_t i = 0; i < listsize; i++, didx++) {
			reg_list[didx].exist = rlist[i].exist;
			reg_list[didx].name = rlist[i].name;
			reg_list[didx].size = 32;
			reg_list[didx].value = calloc(1, sizeof(uint32_t));
			reg_list[didx].dirty = 0;
			reg_list[didx].valid = 0;
			...
			//LOG_DEBUG("POPULATE %-16s list %d exist %d, idx %d, type %d, dbreg_num 0x%04x",
	...
	(*cache_p) = reg_cache;
    LOG_DEBUG("xtensa->total_regs_num %d reg_list_size %d xtensa->dbregs_num %d", 
	...
	if (xtensa_empty_regs == NULL) {
		LOG_ERROR("ERROR: Out of memory");
		return ERROR_FAIL;
	}
	for (uint32_t i = 0; i < xtensa->dbregs_num; i++) {
		...
		xtensa_empty_regs[i].value = calloc(1, sizeof(uint32_t));
...
static int32_t xtensa_gdbqc_parse_exec_tie_ops(struct target *target, char *opstr)
{
	...
	while (opstr && (*opstr == ':')) {
		...
		if (oplen > 32) {
			LOG_ERROR("TIE access instruction too long (%d)\n", oplen);
			break;
		}
		uint32_t i = 0;
		...
		if (i != oplen) {
			LOG_ERROR("TIE access instruction malformed (%d)\n", i);
			break;
		}
...
/* Macro logs an error message, writes to (*response_p) and exits the caller */
#define XTENSA_GDBQC_ENFORCE(exp, code, msg)                 \
	do {                                                     \
		if (!(exp)) {                                        \
			LOG_ERROR(msg);                                  \
			strcpy((*response_p), (xt_qerr[(code)].chrval)); \
			return (xt_qerr[(code)].intval);                 \
		}                                                    \
	} while (0)
...
static int xtensa_gdbqc_qxtreg(struct target *target, const char *packet, char **response_p)
{
	 ...
	 */
	XTENSA_GDBQC_ENFORCE((xtensa->spill_buf), XT_QERR_FAIL, 
			"Spill location not specified. Try 'target remote <host>:3333 &spill_location0'\n");

	char *delim;
	uint32_t regnum = strtoul(packet+6, &delim, 16);
	XTENSA_GDBQC_ENFORCE((*delim == ':'), XT_QERR_INVAL, "Malformed qxtreg packet");
	...
	XTENSA_GDBQC_ENFORCE((reglen*2+1 <= XTENSA_QUERYPKT_RESP_MAX), XT_QERR_MEM, "TIE register too large");
	...
	XTENSA_GDBQC_ENFORCE((status == ERROR_OK), XT_QERR_MEM, "Spill memory save");
	if (iswrite) {
		/* Extract value and store in spill memory */
		uint32_t b = 0;
		char *valbuf = strchr(delim, '=');
		XTENSA_GDBQC_ENFORCE((valbuf && (*valbuf == '=')), XT_QERR_INVAL, "Malformed Qxtreg packet");
		valbuf++;
		...
		XTENSA_GDBQC_ENFORCE((b == reglen), XT_QERR_INVAL, "Malformed Qxtreg packet");
		status = xtensa_write_memory(target, xtensa->spill_loc, memop_size,
						reglen/memop_size, regbuf);
		XTENSA_GDBQC_ENFORCE((status == ERROR_OK), XT_QERR_MEM, "TIE value store");
	}
	...
	if (tieop_status == ERROR_OK) {
		if (iswrite) {
			...
			*(*response_p+2*i) = '\0';
			LOG_DEBUG("TIE response: %s", *response_p);
		}
	}
	...
	status = xtensa_write_memory(target, xtensa->spill_loc, memop_size, 
					xtensa->spill_bytes/memop_size, xtensa->spill_buf);
	XTENSA_GDBQC_ENFORCE((status == ERROR_OK), XT_QERR_MEM, "Spill memory restore");
	XTENSA_GDBQC_ENFORCE((tieop_status == ERROR_OK), XT_QERR_FAIL, "TIE execution");
	return ERROR_OK;
}
...
int xtensa_gdb_query_custom(struct target *target, const char *packet, char **response_p)
{
    ...
    if (!packet || !response_p) {
        LOG_ERROR("invalid parameter: packet %p response_p %p", packet, response_p);
        return ERROR_FAIL;
    }
    ...
    *response_p = xtensa_qpkt_resp;
    if (strncmp(packet, "qxtn", 4) == 0) {
        ...
        return ERROR_OK;
    } else if ((strncmp(packet, "Qxtsis=", 7) == 0) ||
               (strncmp(packet, "Qxtsds=", 7) == 0)) {
		...
		if ((cachep->line_size != line_size) ||
			...
			LOG_WARNING("%cCache mismatch; check xtensa-core-XXX.cfg file",
				cachep == &xtensa->core_config->icache ? 'I' : 'D');
		}
		strcpy(*response_p, "OK");
		return ERROR_OK;
	} else if ((strncmp(packet, "Qxtiram=", 8) == 0) ||
               (strncmp(packet, "Qxtirom=", 8) == 0)) {
		...
		do {
			...
			LOG_DEBUG("memcheck: %dB @ 0x%08x", size, base);
			...
			if (i == memp->count) {
				LOG_WARNING("%s mismatch; check xtensa-core-XXX.cfg file",
		...
		if (!xtensa->core_config->high_irq.enabled ||
			...
			LOG_WARNING("EXCM_LEVEL mismatch; check xtensa-core-XXX.cfg file");
		}
		strcpy(*response_p, "OK");
		return ERROR_OK;
		...
		XTENSA_GDBQC_ENFORCE((*delim == ':'), XT_QERR_INVAL, "Malformed Qxtspill packet");
		...
		XTENSA_GDBQC_ENFORCE((xtensa->spill_buf), XT_QERR_MEM, "Spill buf alloc");
		LOG_DEBUG("Set spill 0x%08x (%d)", xtensa->spill_loc, xtensa->spill_bytes);
        strcpy(*response_p, "OK");
        return ERROR_OK;
    ...
    LOG_WARNING("Unknown target-specific query packet: %s", packet);
	strcpy(*response_p, "");
    return ERROR_OK;
}
...
int xtensa_target_init(struct command_context *cmd_ctx, struct target *target)
{
	...
	xtensa->hw_brps =
		calloc(XT_HW_IBREAK_MAX_NUM, sizeof(struct breakpoint *));
	if (xtensa->hw_brps == NULL) {
		LOG_ERROR("Failed to alloc memory for HW breakpoints!");
		return ERROR_FAIL;
	}
	xtensa->hw_wps =
		calloc(XT_HW_DBREAK_MAX_NUM, sizeof(struct watchpoint *));
...
COMMAND_HELPER(xtensa_cmd_exe_do, struct target *target)
{
	...
	LOG_DEBUG("execute stub: %s", CMD_ARGV[0]);
	xtensa_queue_exec_ins_wide(xtensa, ops, oplen);	/* Handles endian-swap */
	status = xtensa_dm_queue_execute(&xtensa->dbg_mod);
	status |= xtensa_core_status_check(target);
	if (status != ERROR_OK) {
	...
	if (xtensa_fetch_all_regs(target) != ERROR_OK) {
		LOG_ERROR("%s: Failed to fetch register cache (post-exec).", target_name(target));
	}
...
#define XTENSA_CMD_XTOPT_ENFORCE_VAL(opt, val, min, max)           \
	do {                                                           \
		if (((signed)(val) < (min)) || ((val) > (max))) {          \
...
COMMAND_HELPER(xtensa_cmd_xtopt_do, struct xtensa *xtensa)
{
	...
	unsigned opt_val = strtoul(CMD_ARGV[1], NULL, 0);
	if (strcasecmp(opt_name, "bigendian") == 0) {
		XTENSA_CMD_XTOPT_ENFORCE_VAL("bigendian", opt_val, 0, 1);
		...
		XTENSA_CMD_XTOPT_ENFORCE_VAL("ibreaknum", opt_val, 0, 2);
		xtensa->core_config->debug.ibreaks_num = opt_val;
	} else if (strcasecmp(opt_name, "dbreaknum") == 0) {
		XTENSA_CMD_XTOPT_ENFORCE_VAL("dbreaknum", opt_val, 0, 2);
		xtensa->core_config->debug.dbreaks_num = opt_val;
	} else if (strcasecmp(opt_name, "trace") == 0) {
		// TODO: enable trace
...
COMMAND_HELPER(xtensa_cmd_permissive_mode_do, struct xtensa *xtensa)
{
	...
	if (!strcasecmp(CMD_ARGV[0], "off"))
		xtensa->permissive_mode = false;
	else if (!strcasecmp(CMD_ARGV[0], "on"))
		xtensa->permissive_mode = true;
	else return ERROR_COMMAND_SYNTAX_ERROR;	/* "on" or "off" only */
	return ERROR_OK;
}
...
/* dm offset <value> */
COMMAND_HELPER(xtensa_cmd_dm_offset_do, struct xtensa *xtensa)
{
	if (CMD_ARGC != 1) {
		return ERROR_COMMAND_SYNTAX_ERROR;
	}

	uint32_t offset = strtoul(CMD_ARGV[0], NULL, 0);
	return xtensa_dm_set_offset(&xtensa->dbg_mod, offset);
}

COMMAND_HANDLER(xtensa_cmd_dm_offset)
{
	return CALL_COMMAND_HANDLER(xtensa_cmd_dm_offset_do,
		target_to_xtensa(get_current_target(CMD_CTX)));
}

static const struct command_registration xtensa_dm_command_handlers[] = {
	{
		.name = "offset",
		.handler = xtensa_cmd_dm_offset,
		.mode = COMMAND_ANY,
		.help = "Set Debug Module register offset (typically on APB)",
		.usage = "<offset>",
	},
...
static const struct command_registration xtensa_any_command_handlers[] = {
	...
	},
	{
		...
		.mode = COMMAND_ANY,
		.help = "Attempt all Xtensa memory accesses",
		.usage = "['on'|'off']",
	},
	{
		.name = "maskisr",
	...
	},
	{
		.name = "dm",
		.mode = COMMAND_ANY,
		.help = "Xtensa Debug Module command group",
		.usage = "",
		.chain = xtensa_dm_command_handlers,
	},
#if 0
	{
		.name = "perfmon_enable",
